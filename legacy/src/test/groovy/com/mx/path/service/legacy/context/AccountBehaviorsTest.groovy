...
class AccountBehaviorsTest extends Specification implements WithEncryptionService {
  ...
  def "accountBehaviorsWriteReadSession"() {
    ...
    def idMappings = new HashMap<AccountBehavior.ServiceIdentifier, String>()
    def accountOwners = new ArrayList<SessionAccountOwner>()
    idMappings.put(AccountBehavior.ServiceIdentifier.AFCUWS, "123account")
    ...
    then:
    "123account" == AccountBehaviors.loadFromSession().get("account123").getMappedAccountIds().get(AccountBehavior.ServiceIdentifier.AFCUWS)
  }
  ...
  def "verifyServiceAccountIdMapsToMdxAccountId"() {
    ...
    def idMappings = new HashMap<AccountBehavior.ServiceIdentifier, String>()
    idMappings.put(AccountBehavior.ServiceIdentifier.AFCUWS, serviceAccountId)
    accountBehavior.setMappedAccountIds(idMappings)
    ...
    then:
    mdxAccountId == AccountBehaviors.loadFromSession().serviceAccountIdToAccountId(AccountBehavior.ServiceIdentifier.AFCUWS, serviceAccountId)
  }
  ...
  def "verifyNullOnExternalGuidLookup"() {
    ...
    expect:
    AccountBehaviors.loadFromSession().serviceAccountIdToAccountId(AccountBehavior.ServiceIdentifier.AFCUWS, badServiceAccountId) == null
  }
}
